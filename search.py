...
def graph_search(problem, fringe):
    ...
    generatedNodes = 1 # Suponer que el grafo nunca está vacío
    visitedNodes = 0

    start_time = time.perf_counter() * 1000  # Registro del tiempo de inicio en milisegundos

    fringe.append(Node(problem.initial))
    
    while fringe:
        node = fringe.pop()
        # Se incrementa el marcador de nodos visitados
        visitedNodes += 1
        if problem.goal_test(node.state):
            end_time = time.perf_counter() * 1000  # Registro del tiempo de finalización en milisegundos
            elapsed_time = end_time - start_time   # Calcula el tiempo transcurrido en milisegundos
            r = f"Generados: {generatedNodes}\nVisitados: {visitedNodes}\nCosto total: {node.path_cost}\nTiempo total: {elapsed_time} (ms)"
            print(r)
            return node
        if node.state not in closed:
            closed[node.state] = True
            sucessors = node.expand(problem)
            fringe.extend(sucessors)
            # Incrementar nodos generados
            generatedNodes += len(sucessors)
    return None
