...
class BnB(Queue):
    """ Queue that implements fundamentals of Branch and Bounding """
    def __init__(self):
        self.A = []
        self.start = 0
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A = sorted(self.A, key = lambda i: i.path_cost, reverse=False) 
...
class BnB_sub(Queue):
    """ Queue that implements fundamentals of Branch and Bounding with subestimation """
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A = sorted(self.A, key = lambda i: i.path_cost + self.sub.h(i), reverse=False) 
